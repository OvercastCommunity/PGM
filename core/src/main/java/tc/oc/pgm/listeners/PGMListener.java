...
import tc.oc.util.bukkit.NullCommandSender;
  ...
  public void onPrePlayerLogin(final AsyncPlayerPreLoginEvent event) {
    ...
    if (!mm.getMatches().hasNext()) {
      event.disallow(
          ...
          Translations.get().translate("incorrectWorld.kickMessage", NullCommandSender.INSTANCE));
    }
  }
  ...
  public void onPlayerLogin(final PlayerLoginEvent event) {
    ...
    if (event.getResult() == Result.KICK_FULL) {
      if (event.getPlayer().hasPermission(Permissions.JOIN_FULL)) {
        ...
        event.setKickMessage(Translations.get().translate("serverFull", event.getPlayer()));
      }
    }
  }
  ...
  public void addPlayerOnJoin(final PlayerJoinEvent event) {
    if (this.mm.getMatch(event.getWorld()) == null) {
      event
          ...
          .kickPlayer(
              ChatColor.RED
                  + Translations.get().translate("incorrectWorld.kickMessage", event.getPlayer()));
      this.parent
          .getLogger()
          .info(
              "Had to kick player "
