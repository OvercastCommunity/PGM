...
import tc.oc.util.named.NameStyle;
...
public class AntiGriefListener implements Listener {
  ...
  private void participantDefuse(Player player, Entity entity) {
    ...
    if (block != null
      ...
      clicker.sendMessage(
          ChatColor.RED + Translations.get().translate("defuse.water", clicker.getBukkit()));
      return;
    }
    ...
    if (owner == null
      ...
      if (owner != null) {
        ...
        this.notifyDefuse(
            clicker,
            entity,
            ChatColor.RED + Translations.get().translate("defuse.world", clicker.getBukkit()));
      }
    }
  }
  ...
  public void giveKit(final ObserverKitApplyEvent event) {
    ...
    meta.setDisplayName(
        Translations.get().translate("defuse.displayName", event.getPlayer().getBukkit()));
    meta.setLore(
        Lists.newArrayList(
            ChatColor.GRAY
                + Translations.get().translate("defuse.tooltip", event.getPlayer().getBukkit())));
