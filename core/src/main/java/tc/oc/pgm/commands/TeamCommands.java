...
public class TeamCommands {
  ...
  public static void myTeam(CommandSender sender, MatchPlayer player) throws CommandException {
    if (player.getParty() instanceof Team) {
      ...
      throw new CommandException(
          Translations.get().translate("command.gameplay.myteam.notOnTeam", sender));
    }
  }
  ...
  public static void shuffle(CommandSender sender, TeamMatchModule tmm, Match match)
    ...
    if (match.isRunning()) {
      throw new CommandException(
          Translations.get().translate("command.team.shuffle.matchRunning", sender));
    } else {
  ...
  public static void alias(
    ...
    if (team == null) {
      throw new CommandException(Translations.get().translate("command.teamNotFound", sender));
    }
    ...
    for (Team t : tmm.getTeams()) {
      if (t.getName().equalsIgnoreCase(newName)) {
        throw new CommandException(
            Translations.get().translate("command.team.alias.nameAlreadyUsed", sender, newName));
      }
    }
