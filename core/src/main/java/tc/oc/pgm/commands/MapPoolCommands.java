...
import tc.oc.util.named.MapNameStyle;
import tc.oc.util.translations.Translations;
...
public class MapPoolCommands {
  ...
  public static void rotation(
    ...
    if (mapPool == null) {
      sender.sendMessage(
          ChatColor.RED + Translations.get().translate("command.pools.noPoolMatch", sender));
      return;
    }
    ...
    String title = Translations.get().translate("command.pools.mapPool.title", sender);
    title +=
  ...
  public static void rotations(
    ...
    if (mapPools.isEmpty()) {
      sender.sendMessage(
          ChatColor.RED + Translations.get().translate("command.pools.noMapPools", sender));
      return;
    }
    ...
    String title = Translations.get().translate("command.pools.mapPoolList.title", sender);
    title = ComponentUtils.paginate(title, page, pages);
    title = ComponentUtils.horizontalLineHeading(title, ChatColor.BLUE, 250);
  ...
  public static void skip(CommandSender sender, MapOrder mapOrder, @Default("1") int positions)
    ...
    if (positions < 0) {
      sender.sendMessage(
          ChatColor.RED + Translations.get().translate("command.pools.skip.noNegative", sender));
      return;
    }
    ...
    if (!(pool instanceof Rotation)) {
      sender.sendMessage(
          ChatColor.RED + Translations.get().translate("command.pools.noRotation", sender));
      return;
    }
  ...
  public static void voteNext(
    ...
    if (poll == null) {
      sender.sendMessage(
          ChatColor.RED + Translations.get().translate("command.pool.vote.noVote", sender));
      return;
    }
  ...
  private static MapPoolManager getMapPoolManager(CommandSender sender, MapOrder mapOrder)
    ...
    throw new CommandException(
        Translations.get().translate("command.pools.mapPoolsDisabled", sender));
  }
}
