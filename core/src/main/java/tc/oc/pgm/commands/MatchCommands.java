...
public class MatchCommands {
  ...
  public static void matchInfo(CommandSender sender, MatchPlayer player, Match match) {
    ...
    if (haveGameInfo) {
      ...
      sender.sendMessage(
          ChatColor.DARK_PURPLE
              + Translations.get().translate("command.match.matchInfo.time", sender)
              + ": "
              + ChatColor.GOLD
    ...
    if (tmm != null) {
      ...
      teamCountParts.add(
          ChatColor.YELLOW
              + Translations.get().translate("command.match.matchInfo.players", sender)
              + ChatColor.GRAY
              + ": "
              + ChatColor.WHITE
    ...
    teamCountParts.add(
        ChatColor.AQUA
            + Translations.get().translate("command.match.matchInfo.observers", sender)
            + ChatColor.GRAY
            + ": "
            + ChatColor.WHITE
    ...
    if (haveGameInfo && gmm != null) {
      if (tmm != null && gmm.getGoalsByCompetitor().size() > 0) {
        ...
        if (!teamGoalTexts.isEmpty()) {
          sender.sendMessage(
              ChatColor.DARK_PURPLE
                  + Translations.get().translate("command.match.matchInfo.goals", sender)
                  + ":");
