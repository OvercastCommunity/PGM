...
public class StartCommands {
  ...
  public static void start(CommandSender sender, Match match, Duration countdown, Duration huddle)
    ...
    if (match.isRunning()) {
      throw new CommandException(
          Translations.get().translate("command.admin.start.matchRunning", sender));
    }
    if (match.isFinished()) {
      throw new CommandException(
          Translations.get().translate("command.admin.start.matchFinished", sender));
    }
  ...
  public static void autostart(CommandSender sender, Match match, @Nullable String toggle)
    ...
    if (autoStart) {
      sender.sendMessage(
          ChatColor.GREEN + Translations.get().translate("command.admin.autoStartEnabled", sender));
      smm.autoStartCountdown();
    } else {
      sender.sendMessage(
          ChatColor.BLUE + Translations.get().translate("command.admin.autoStartDisabled", sender));
    }
  }
}
